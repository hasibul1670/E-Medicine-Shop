...
const DoctorCard = ({doctor}: {doctor: Doctor}) => {
  ...
  return (
    <Stack direction="row" gap={2}>
      <Stack direction="row" flex={1} gap={3} sx={{height: 235, bgcolor: "white", p: 3}}>
        ...
        <Stack flex={1} justifyContent="space-between">
          <Box sx={{flex: 1}}>
            ...
            <Typography sx={{my: "2px", color: "secondary.main"}}>{doctor?.designation}</Typography>
      ...
      <Stack sx={{height: 235, bgcolor: "white", width: "400px", p: 3}}>
        <Box flex={1}>
          ...
          <Typography sx={{fontWeight: "600", mt: "3px"}}>{doctor?.currentWorkingPlace}</Typography>
        </Box>
        <Box
          sx={{
            borderBottom: "2px dashed",
